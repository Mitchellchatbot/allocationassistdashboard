/**
 * backfill-hospital-images — team feedback #9 ("Hospital image should actually
 * appear in the sent Working Opportunity email").
 *
 * The WO email renders `hospitals.image_url` as a hero <img> under each
 * opportunity (see _shared/doctor-working-op.ts). The whole send pipeline —
 * SendProfileDialog preview, send-flow-email and send-batch — already passes
 * image_url through, so the ONLY reason an image doesn't appear is that the
 * column is blank: the original hospital photos live in an M365 mailbox nobody
 * has migrated. This backfill fills the gap WITHOUT that mailbox by pulling
 * each hospital's website Open-Graph image (`og:image`, then `twitter:image`,
 * then `<link rel="image_src">`) — usually a building / hero shot, exactly the
 * kind of picture the team wants. `hospitals.website` is populated first by the
 * sibling `backfill-hospital-websites` function.
 *
 ...
 *   body: { apply?: boolean, overwrite?: boolean, limit?: number, only?: string[] }
 *     apply     — false (default) = dry run; true = perform the updates.
 *     overwrite — false (default) = only fill BLANK image_url; true = also
 *                 replace a hospital's existing image_url.
 *     limit     — cap how many websites to fetch this run (default 100).
 *     only      — optional list of hospital ids to restrict the run to.
 *
 * Non-destructive, idempotent (re-running skips hospitals already imaged unless
...
serve(async (req) => {
  ...
  const { data: hospitals, error: hErr } = await supabase
    .from("hospitals").select("id, name, website, image_url");
  if (hErr) return json({ ok: false, error: `hospitals read failed: ${hErr.message}` }, 500);

  // Candidates: website present, and (image blank OR overwrite), optionally
  // restricted to `only` ids.
  const candidates = (hospitals ?? []).filter((h) => {
    ...
    const hasImage = String(h.image_url ?? "").trim().length > 0;
    return overwrite || !hasImage;
  });

  const toUpdate: Array<{ id: string; name: string; website: string; from: string | null; to: string }> = [];
  const noImageFound: Array<{ name: string; website: string }> = [];
  const skippedNoWebsite = (hospitals ?? []).filter(h => !normWebsite(String(h.website ?? ""))).length;
  const alreadyHave = (hospitals ?? []).filter(h => !overwrite && String(h.image_url ?? "").trim().length > 0).length;
  ...
  for (const h of candidates) {
    ...
    if (!html) { noImageFound.push({ name: String(h.name ?? ""), website }); continue; }
    const image = extractImage(html, finalUrl);
    if (!image) { noImageFound.push({ name: String(h.name ?? ""), website }); continue; }
    const current = String(h.image_url ?? "").trim();
    if (current && current === image) continue; // idempotent
    toUpdate.push({ id: String(h.id), name: String(h.name ?? ""), website, from: current || null, to: image });
  }
  ...
  if (apply) {
    for (const u of toUpdate) {
      const { error } = await supabase
        .from("hospitals")
        .update({ image_url: u.to, updated_at: new Date().toISOString() })
        .eq("id", u.id);
  ...
  return json({
    ...
    summary: {
      ...
      no_image_found: noImageFound.length,
      skipped_no_website: skippedNoWebsite,
      already_have_image: alreadyHave,
