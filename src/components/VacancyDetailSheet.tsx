...
import { UserSquare, Mail, Phone, Plus, ClipboardList, Sparkles, Globe, Pencil, Save, X } from "lucide-react";
import { toast } from "sonner";
import {
  useMatchingDoctors, useVacancyLinks,
  useLinkLeadToVacancy, useUpdateVacancy,
  type Vacancy, type VacancyStatus, type VacancyPriority,
} from "@/hooks/use-vacancies";
import { useHospitals } from "@/hooks/use-hospitals";
import { usePublishedWpCandidates, type WpCandidate } from "@/hooks/use-wp-candidates";
...
export function VacancyDetailSheet({ vacancy, open, onClose }: Props) {
  ...
  // Global WordPress-doctor search — the *currently-published* WP candidates
  // (doctors actively looking for jobs), NOT the onboarded roster. Sales types a
  // name/specialty and can introduce anyone here to this vacancy's hospital.
  const { data: wpAll = [] } = usePublishedWpCandidates();
  const [wpQuery, setWpQuery] = useState("");
  const wpResults = useMemo(() => {
    ...
    return wpAll
      .filter(c =>
        ...
        (c.subspecialty ?? "").toLowerCase().includes(q) ||
  ...
  return (
    ...
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="w-full sm:max-w-[640px] overflow-y-auto">
        ...
        {/* ── Onboarded matches (auto-scored from the AA roster) ────────
            Leads tab removed (Hasan 2026-07-06) — the vacancy sheet now
            surfaces just the auto-ranked onboarded doctors plus a global
            WordPress-doctor search below. `existing`/`linkedIds` are still
            used to render the "Linked" badge on already-linked matches. */}
        ...
        {/* ── Global WordPress-doctor search ───────────────────────────
            Doctors published on WordPress right now (actively job-seeking),
            NOT the onboarded roster. Sales searches by name/specialty and
            introduces any of them to this vacancy's hospital in one click. */}
        <section className="mt-6 space-y-2">
          <div className="flex items-center gap-1.5 text-[12px] font-medium">
            <Globe className="h-3.5 w-3.5 text-sky-600" />
            Search WordPress doctors
          </div>
          <p className="text-[11px] text-muted-foreground">
            Live candidates on allocationassist.com looking for jobs — search any of them and introduce them to {vacancy.hospital_name}.
          </p>
          <Input
            value={wpQuery}
          ...
          {wpQuery.trim() && wpResults.length === 0 && (
            <div className="rounded-md border border-dashed py-5 text-center text-[12px] text-muted-foreground">
              No published WordPress doctors match "{wpQuery.trim()}".
            </div>
          )}
          ...
          {wpResults.length > 0 && (
            <div className="space-y-1.5 max-h-[340px] overflow-y-auto pr-0.5">
              {wpResults.map(c => (
                <div key={c.id} className="flex items-center gap-2 rounded-md border bg-white px-2.5 py-1.5">
                  <UserSquare className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-medium truncate">{c.full_name || c.email || "Candidate"}</div>
                    <div className="text-[10px] text-muted-foreground truncate">
                      {[c.specialty, c.subspecialty, c.nationality].filter(Boolean).join(" · ") || "—"}
                    </div>
                  </div>
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 rounded-md border border-teal-200 bg-teal-50 px-1.5 py-0.5 text-[10px] font-medium text-teal-700 hover:bg-teal-100 transition-colors shrink-0"
                    title={`Introduce ${c.full_name ?? "this doctor"} to ${vacancy.hospital_name} — opens the Send Profile flow`}
                    onClick={() => onEmailWp(c)}
                  >
