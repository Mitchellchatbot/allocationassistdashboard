...
import { pipelineStages, salesMetrics, topRecruiters, stageConversion } from "@/lib/mock-data";
...
const Sales = () => (
  <DashboardLayout title="Sales & Pipeline" subtitle="Doctor placement pipeline and recruiter performance">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {pipelineStages.map((stage, i) => (
            <div key={stage.stage} className="flex items-center gap-1.5">
              ...
              {i < pipelineStages.length - 1 && <ArrowRight className="h-3 w-3 text-border shrink-0" />}
            </div>
          ))}
        </div>
      </CardContent>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-5">
      <Card className="shadow-sm border-border/50">
        ...
        <CardContent className="px-4 pb-4 space-y-2">
          {[
            { label: "Placements (Quarter)", val: salesMetrics.dealsClosed },
            { label: "Conversion Rate", val: `${salesMetrics.conversionRate}%` },
            { label: "Avg Processing Time", val: salesMetrics.avgCycleTime },
          ].map(m => (
      ...
      <Card className="shadow-sm border-border/50">
        ...
        <CardContent className="px-4 pb-4 space-y-2">
          {[
            { icon: Phone, label: "Outbound Calls", val: salesMetrics.outboundCalls.toLocaleString(), color: "text-primary" },
            { icon: Mail, label: "Emails Sent", val: salesMetrics.emailsSent.toLocaleString(), color: "text-info" },
            { icon: Clock, label: "Follow-ups Pending", val: salesMetrics.followUpsPending.toString(), color: "text-warning" },
          ].map(m => (
    ...
    <Card className="shadow-sm border-border/50">
      <CardHeader className="pb-1 pt-4 px-4">
        <CardTitle className="text-[12px] font-medium text-muted-foreground uppercase tracking-wide">Top Recruiters</CardTitle>
      </CardHeader>
      <CardContent className="px-4 pb-4">
        <Table>
          <TableHeader>
