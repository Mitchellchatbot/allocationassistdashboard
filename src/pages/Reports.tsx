...
function KpiStrip({ bundle }: { bundle: ReturnType<typeof useReportingMetrics> }) {
  ...
  const tiles: Array<{
    ...
    // related milestones rather than three different states.
    {
      ...
      onClickThrough: () => navigate("/doctor-profiles"),
      drilldown: <LifecycleList rows={drilldowns.signed} milestone="signed_at" onJump={(id) => navigate(`/doctor-profiles?id=${encodeURIComponent(id)}`)} />,
    },
    {
      ...
      onClickThrough: () => navigate("/doctor-profiles"),
      drilldown: <LifecycleList rows={drilldowns.joined} milestone="joined_at" onJump={(id) => navigate(`/doctor-profiles?id=${encodeURIComponent(id)}`)} />,
    },
    {
      ...
      onClickThrough: () => navigate("/doctor-profiles"),
      drilldown: <LifecycleList rows={drilldowns.paid} milestone="paid_at" onJump={(id) => navigate(`/doctor-profiles?id=${encodeURIComponent(id)}`)} />,
    },
  ];
