 ...
 */
import { createContext, useCallback, useContext, useEffect, useMemo, useRef, useState, lazy, Suspense, type ReactNode, type KeyboardEvent } from "react";
import { useLocation } from "react-router-dom";
import { Sparkles, Send, RotateCcw, X, RefreshCw, Copy } from "lucide-react";
import { toast } from "sonner";
// react-markdown drags in the whole micromark/mdast/unified stack (~400 kB of
// source). Lazy-load it so that weight only downloads when the AI panel first
// renders a message, not in the main every-page bundle.
                          ...
                          <div className="space-y-2">
                            {cleanText && (
                              <div className="group relative rounded-2xl rounded-bl-sm bg-card border border-border/50 px-4 py-3">
                                ...
                                <Suspense fallback={<p className="text-[13px] text-muted-foreground whitespace-pre-wrap">{cleanText}</p>}>
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="prose prose-sm max-w-none text-[13px] leading-relaxed text-foreground/90 [&_p]:my-1 [&_h2]:text-[14px] [&_h2]:font-semibold [&_h2]:mt-3 [&_h2]:mb-1 [&_ul]:my-1 [&_li]:my-0.5 [&_strong]:font-semibold [&_strong]:text-foreground">
                        <Suspense fallback={<span className="whitespace-pre-wrap">{streaming}</span>}>
                          <ChatMarkdown content={streaming} plain />
                        </Suspense>
                      </div>
